...
<style>
/* ================= PREMIUM OBISIDIAN CARBON UI DESIGN ================= */
:root {
    --primary: #3b82f6;
    ...
    /* لون الخلفية الاحترافي الفاخر الجديد (Carbon Deep Slate) */
    --bg-app: #0f1319; 
    --surface-card: #151b23;
    --input-bg: #0b0f13;
...
/* ================= مكونات الواجهة والبطاقات العريضة ================= */
.app-header {
    display: flex;
...
.btn-secondary { background: #262f3c; color: var(--text-main); border: 1px solid var(--border); }
.btn-secondary:hover { background: #2f3a4a; }

/* عناصر الإدخال المحدثة */
.grid-inputs {
    display: grid;
...
/* ================= تصميم الجداول الاحترافي العريض ================= */
.table-container {
    overflow-x: auto;
...
th { background: #0b0f13; color: var(--text-muted); font-weight: 700; border-bottom: 2px solid var(--border); }
td { background: var(--surface-card); border-bottom: 1px solid var(--border); font-weight: 600; }
...
.stat-icon {
    width: 48px; height: 48px; border-radius: var(--radius-md); display: flex; align-items: center; justify-content: center; font-size: 20px;
}
.stat-card.primary .stat-icon { background: rgba(59, 130, 246, 0.08); color: #60a5fa; }
.stat-card.success .stat-icon { background: rgba(16, 185, 129, 0.08); color: #34d399; }
.stat-card.danger .stat-icon { background: rgba(239, 68, 68, 0.08); color: #f87171; }
.stat-card.purple .stat-icon { background: rgba(168, 85, 247, 0.08); color: #c084fc; }
...
.basket-items { max-height: 280px; overflow-y: auto; margin-bottom: 20px; }

.basket-row {
    display: flex; justify-content: space-between; align-items: center; padding: 12px 15px; background: var(--input-bg); border: 1px solid var(--border); border-radius: var(--radius-md); margin-bottom: 10px;
}

/* ستايل حالات الخطورة في السلع الناقصة */
.row-empty { border-right: 5px solid var(--danger); }
.row-danger { border-right: 5px solid rgba(239, 68, 68, 0.6); }
.row-warning { border-right: 5px solid var(--warning); }
.row-notice { border-right: 5px solid var(--primary); }

::-webkit-scrollbar { width: 6px; height: 6px; }
::-webkit-scrollbar-track { background: var(--bg-app); }
::-webkit-scrollbar-thumb { background: #262f3c; border-radius: 10px; }
</style>
</head>
...
<div class="main-container">
    ...
    <!-- الهيدر الرئيسي الاحترافي مع أزرار التحكم بالنسخ الاحتياطية -->
    <div class="app-header">
        <div class="app-title">
            <i class="fa-solid fa-bolt"></i>
            <span>نظام المبيعات الذكي الاحترافي</span>
        </div>
        <div style="display: flex; gap: 10px;">
            <button class="btn-secondary" onclick="exportData()" style="font-size:13px; padding:10px 16px;"><i class="fa-solid fa-file-export"></i> تصدير نسخة احتياطية</button>
            <button class="btn-primary" onclick="triggerImport()" style="font-size:13px; padding:10px 16px; background:var(--purple-gradient);"><i class="fa-solid fa-file-import"></i> استيراد نسخة احتياطية</button>
        </div>
    </div>
    ...
    <!-- القسم 1: لوحة التحكم العامة العريضة -->
    <div class="card-box">
        <h2 class="section-title"><i class="fa-solid fa-chart-pie" style="color:var(--primary);"></i> الإحصاءات العامة الحالية للنظام</h2>
    ...
    <!-- القسم 2: إدارة المنتجات والمخزن -->
    <div class="card-box">
        <h2 class="section-title"><i class="fa-solid fa-box-open" style="color:var(--primary);"></i> إدارة المنتجات والمخزن الكلي</h2>
        
        <div style="background: rgba(255,255,255,0.01); border: 1px solid var(--border); padding: 20px; border-radius: var(--radius-lg); margin-bottom: 25px;">
            <h3 style="font-size:14px; margin-bottom:15px; color:var(--success);"><i class="fa-solid fa-plus-circle"></i> إضافة منتج جديد يدوياً أو بالباركود</h3>
            ...
            <button class="btn-success" onclick="addProduct()" style="width:100%; height:45px; margin-top:10px;"><i class="fa-solid fa-check"></i> إضافة المنتج وتثبيته بالمخزن</button>
        </div>

        <div style="margin-bottom: 15px;">
            <input id="productSearch" placeholder="🔍 ابحث سريعاً بالاسم أو الباركود لفلترة جدول السلع أدناه..." oninput="renderProducts()" style="padding:14px;">
        </div>
    ...
    <!-- القسم 3: واجهة البيع السريعة والتعديل الفوري الفاتورة -->
    <div class="card-box">
        <h2 class="section-title"><i class="fa-solid fa-cash-register" style="color:var(--primary);"></i> واجهة بيع السلع وإلغاء وتعديل الفواتير</h2>
        
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap:20px; margin-bottom:25px;">
            <div style="background:var(--input-bg); border:1px solid var(--border); padding:15px; border-radius:var(--radius-lg); display:flex; align-items:center; justify-content:space-around;">
                <span style="font-weight:700; font-size:14px;"><i class="fa-solid fa-receipt" style="color:var(--primary);"></i> رقم الطلب الحالي:</span>
                <input id="cmdNumberInput" type="number" min="1" style="width:100px; text-align:center; font-weight:800; padding:8px;" oninput="updateCommandNumberManual()">
            ...
            <div style="background:var(--input-bg); border:1px solid var(--border); padding:15px; border-radius:var(--radius-lg); display:flex; align-items:center; gap:15px;">
                <input id="searchOrderNumber" type="number" placeholder="أدخل رقم طلبية سابقة لتعديلها...">
        ...
        <div class="sales-layout">
            <div>
                <div style="background: rgba(255,255,255,0.01); border: 1px solid var(--border); padding: 20px; border-radius: var(--radius-lg); margin-bottom: 25px;">
                    <h3 style="font-size:14px; margin-bottom:15px; color:var(--primary);"><i class="fa-solid fa-filter"></i> فلترة واختيار المنتج المستهدف بالبيع</h3>
                    ...
                    <button class="btn-primary" onclick="addToCommand()" style="width:100%; height:45px; font-size:14px;"><i class="fa-solid fa-cart-plus"></i> إضافة المنتج المحدد للسلة</button>
                </div>

                <h3 style="font-size:15px; font-weight:700; margin-bottom:15px;"><i class="fa-solid fa-clock-rotate-left"></i> سجل العمليات والفواتير السابقة المقيدة</h3>
                <div id="salesLog" style="max-height:350px; overflow-y:auto;"></div>
            </div>

            <!-- سلة التسوق المتوضعة بجانب المدخلات -->
            <div style="background: var(--surface-card); border: 1px solid rgba(16, 185, 129, 0.2); border-radius: var(--radius-xl); padding: 20px;" class="basket-container">
                <h3 style="font-size:15px; margin-bottom:15px; color:var(--success); border-bottom:1px solid var(--border); padding-bottom:10px;"><i class="fa-solid fa-basket-shopping"></i> سلة التسوق الحالية</h3>
                <div id="currentCommand" class="basket-items"></div>
                
                <div style="background:rgba(16, 185, 129, 0.05); padding:20px; border-radius:var(--radius-md); border:1px solid rgba(16, 185, 129, 0.1); margin-bottom:20px; text-align:left;">
                    <span style="color:var(--success); font-size:13px; font-weight:700;">المجموع الإجمالي المستحق:</span>
                    <h2 id="commandTotal" style="color:var(--success); font-size:26px; font-weight:900; margin-top:3px;">0.00 DA</h2>
                </div>
                <button class="btn-success" onclick="confirmCommand()" style="width:100%; padding:14px; font-size:15px;"><i class="fa-solid fa-circle-check"></i> تأكيد الفاتورة وحفظ البيع</button>
            </div>
        </div>
    </div>

    <!-- القسم 4: إدارة وتدوين المصاريف الكلية للمحل -->
    <div class="card-box">
        <h2 class="section-title"><i class="fa-solid fa-hand-holding-dollar" style="color:var(--danger);"></i> إدارة مصاريف المحل وتقييدها</h2>
        <div style="background: rgba(255,255,255,0.01); border: 1px solid var(--border); padding: 20px; border-radius: var(--radius-lg); margin-bottom: 25px;">
            <div class="form-group" style="margin-bottom:15px;">
            ...
            <button class="btn-danger" onclick="addExpense()" style="width:100%; height:45px; margin-top:10px;"><i class="fa-solid fa-check"></i> حفظ وقيد المصروف بالدفتر المالي</button>
        </div>
        
        <h3 style="font-size:15px; font-weight:700; margin-bottom:15px;"><i class="fa-solid fa-receipt"></i> كشف الدفتر اليومي والشهري للمصاريف المقيّدة</h3>
        <div id="expensesLog" style="max-height:350px; overflow-y:auto;"></div>
    </div>

    <!-- القسم 5: التقارير المالية والأرباح الصافية الحقيقية -->
    <div class="card-box">
        <h2 class="section-title"><i class="fa-solid fa-chart-line" style="color:var(--success);"></i> التقارير والتحليلات المالية والفوائد الصافية</h2>
        
        <div style="background:rgba(168, 85, 247, 0.02); border: 1px solid rgba(168, 85, 247, 0.15); padding: 20px; border-radius: var(--radius-lg); margin-bottom: 25px;">
            <h3 style="font-size:14px; margin-bottom:15px; color:#c084fc;"><i class="fa-solid fa-calendar-days"></i> فرز وحساب الأرباح الصافية خلال فترة زمنية محددة</h3>
            <div class="grid-inputs">
        ...
        <div class="stats-grid">
            <div class="stat-card purple" style="grid-column: span 2 / span 2;">
                ...
                <div class="stat-icon" style="background:rgba(168, 85, 247, 0.05); color:#c084fc;"><i class="fa-solid fa-filter"></i></div>
            </div>
            <div class="stat-card success">
            ...
            <div class="stat-card primary" style="grid-column: span 2 / span 2; border-right:4px solid #fff;">
                <div class="stat-info"><p>👑 صافي فائدة وأرباح السنة الإجمالية الحقيقية</p><h3 id="yearlyProfit" style="color:#fff;">0.00 DA</h3></div>
                <div class="stat-icon" style="background:rgba(255,255,255,0.05); color:#fff;"><i class="fa-solid fa-crown"></i></div>
            </div>
        </div>
    </div>

    <!-- القسم 6: السلع الناقصة وتنبيهات المخاطر المخزنية -->
    <div class="card-box">
        <h2 class="section-title"><i class="fa-solid fa-triangle-exclamation" style="color:var(--warning);"></i> كشف السلع الناقصة بالمحل ومستويات الخطورة</h2>
        <div class="table-container">
            <table>
...
// التنبيهات الصوتية للتفاعل السلس
...
function addProduct(){
    ...
    if(batches.some(b => b.ref === ref)) {
        ...
        alert("خطأ: كود الرفرونس مستعمل مسبقاً لمنتج آخر!"); return;
    }
    if(Number(sell) < Number(buy)) {
        playErrorSound(); sellF.classList.add('input-error');
        alert("خطأ مالي: لا يمكن إدخال سعر بيع أقل من سعر الشراء!"); return;
    }
...
function deleteProduct(id){
    if(confirm("هل أنت متأكد من مسح هذه السلعة نهائياً من النظام؟")) { batches = batches.filter(x => x.id !== id); save(); render(); renderSalesOptions(); }
}
...
function editProduct(id) {
    ...
    let newSell = +prompt("تعديل سعر البيع المقترح:", b.sell) || b.sell;
...
function renderSalesOptions(){
    ...
    saleStock.innerHTML = filtered.map(b => `<option value="${b.id}">[${b.ref}] ${b.name} - المتاح: ${b.qty} قطعة - السعر: ${b.sell} DA</option>`).join("");
...
function addToCommand(){
    ...
    if(b.qty < qty) return alert("المخزون المتوفر غير كافٍ!");
...
function addExpense(){
    ...
    if(!title || !amount || !date) return alert("يرجى تعبئة كافة الحقول");
...
function renderProducts(){
    ...
    <tr>
        ...
        <td><button class="btn-secondary" style="padding:6px 12px; color:#3b82f6;" onclick="editProduct(${b.id})"><i class="fa-solid fa-pen"></i></button></td>
...
function render(){
    ...
    if(oKeys.length === 0) {
        ...
        document.getElementById('salesLog').innerHTML = oKeys.map(num => {
            ...
            `).join("");
            return `
            <div class="card-box" style="padding:15px; margin-bottom:15px; background:#0b0f13;">
                <div style="display:flex; justify-content:space-between; align-items:center; border-bottom:1px dashed var(--border); padding-bottom:8px; margin-bottom:10px;">
                    ...
                    <div style="display:flex; gap:6px;">
                        <button class="btn-secondary" style="padding:4px 8px; font-size:12px;" onclick="document.getElementById('searchOrderNumber').value=${num}; loadOrderToEdit();"><i class="fa-solid fa-edit"></i> جلب</button>
    ...
    document.getElementById('expensesLog').innerHTML = [...expenses].reverse().map(e => {
        ...
        return `
        <div class="card-box" style="padding:15px; margin-bottom:10px; border-right:4px solid var(--danger); display:flex; justify-content:space-between; align-items:center; background:#0b0f13;">
    ...
    if(expenses.length === 0) document.getElementById('expensesLog').innerHTML = "<p style='text-align:center; padding:15px; color:var(--text-muted);'>سجل المصاريف فارغ 🌟</p>";
    
    document.getElementById('homeTotalExpenses').innerHTML = totalExpSum.toFixed(2) + " DA";

    // الأرباح والفوائد الزمنية
...
function importData(event) {
    ...
    if(!confirm("استيراد ملف جديد سيقوم بمسح كافة بياناتك الحالية. هل ترغب بالاستمرار؟")) { event.target.value = ''; return; }
    ...
    reader.onload = function(e) {
        try {
            ...
            if(res.hasOwnProperty('batches') && res.hasOwnProperty('sales')) {
                ...
                save(); render(); renderSalesOptions(); alert("🎉 تم استرجاع النسخة الاحتياطية بنجاح!");
            } else { alert("الملف المرفوع غير متوافق مع بنية النظام الكود الحالي."); }
        } catch(err) { alert("حدث خطأ أثناء قراءة ملف النسخة الاحتياطية."); }
        event.target.value = '';
    };
